(function ($) {
    ...
    $('.sfxexchangerates-select-image').click(function (e) {
        ...
        var _this = $(this);
        var _data = _this.data();
        _data.id = _data.target.replace(/[^a-zA-Z0-9]/g, '');
        ...
        _mediaUploader[_data.id].on('select', function () {
            ...
            if (_data.multiple) {
                var _elements = _mediaUploader[_data.id].state().get('selection').toJSON();
                ...
                var _element = _mediaUploader[_data.id].state().get('selection').first().toJSON();
